...
def upgrade() -> None:
    # ENUMs
    postgresql.ENUM("draft", "validated", "approved", "partially_released", "released", "cancelled", name="production_plan_status").create(op.get_bind())
    postgresql.ENUM("manual", "excel_import", "api", "integration", name="plan_source_type").create(op.get_bind())
    postgresql.ENUM("draft", "invalid", "valid", "approved", "released", "cancelled", name="plan_position_status").create(op.get_bind())
    postgresql.ENUM("pending", "valid", "invalid", name="plan_position_validation_status").create(op.get_bind())
    postgresql.ENUM("auto", "manual_confirmed", "legacy", name="plan_position_route_origin").create(op.get_bind())
    postgresql.ENUM("exact", "corrected", "unknown", name="plan_position_route_match_quality").create(op.get_bind())
    postgresql.ENUM("wildcard_rule", "fallback_first_active", "selection_rules", "no_route_candidate", "route_rule_conflict", "legacy", name="plan_position_route_match_reason").create(op.get_bind())
    postgresql.ENUM("draft", "applied", "cancelled", name="plan_change_set_status").create(op.get_bind())
    postgresql.ENUM("create_position", "update_draft_position", "mark_possible_duplicate", "ignore_unchanged", "cancel_draft_position", name="plan_change_action").create(op.get_bind())
    postgresql.ENUM("pending", "warning", "invalid", "applied", name="plan_change_item_status").create(op.get_bind())
    postgresql.ENUM("near_term", "weekly", "future_preparation", "manual", name="release_batch_type").create(op.get_bind())
    postgresql.ENUM("draft", "released", "cancelled", name="release_batch_status").create(op.get_bind())
    postgresql.ENUM("active", "cancelled", "completed", name="internal_plan_status").create(op.get_bind())
    postgresql.ENUM("waiting_previous", "ready", "in_progress", "partially_completed", "completed", "cancelled", name="work_task_status").create(op.get_bind())
    ...
    op.create_table('production_plans',
        ...
        sa.Column('status', sa.Enum("draft", "validated", "approved", "partially_released", "released", "cancelled", name="production_plan_status"), nullable=False, server_default=sa.text("'draft'")),
    ...
    op.create_table('plan_positions',
        ...
        sa.Column('route_id', sa.BigInteger(), sa.ForeignKey('production_routes.id'), nullable=True),
    ...
    op.create_table('plan_change_sets',
        ...
        sa.Column('production_plan_id', sa.BigInteger(), sa.ForeignKey('production_plans.id'), nullable=False),
        sa.Column('import_batch_id', sa.BigInteger(), sa.ForeignKey('import_batches.id'), nullable=True),
        sa.Column('status', sa.Enum("draft", "applied", "cancelled", name="plan_change_set_status"), nullable=False, server_default=sa.text("'draft'")),
    ...
    op.create_table('release_batches',
        ...
        sa.Column('status', sa.Enum("draft", "released", "cancelled", name="release_batch_status"), nullable=False, server_default=sa.text("'draft'")),
    ...
    op.create_table('internal_plans',
        ...
        sa.Column('status', sa.Enum("active", "cancelled", "completed", name="internal_plan_status"), nullable=False, server_default=sa.text("'active'")),
        sa.Column('created_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
    )
