...
async def test_seed_routes_creates_characteristic_routes(client, session) -> None:
    ...
    data = response.json()
    assert data == {"import_templates": 2, "route_rule_profiles": 1, "routes": 2, "selection_rules": 17, "sections": 12, "section_operations": 21}
...
async def test_force_seed_clears_generated_production_data(client, session) -> None:
    ...
    assert force_response.json() == {"import_templates": 2, "route_rule_profiles": 1, "routes": 2, "selection_rules": 17, "sections": 12, "section_operations": 21}
