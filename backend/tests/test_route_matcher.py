...
async def test_excel_condition_diagnostics_capture_header_mismatch(session) -> None:
    section = Section(code="RAW_STOCK", name="Warehouse", type="raw_stock", is_active=True)
...
async def test_route_rule_conflict_returns_error(session) -> None:
    section = Section(code="DRILLING", name="Drill", type="production", is_active=True)
    session.add(section)
    await session.flush()
    session.add(
        RouteSelectionRule(
...
async def test_select_route_scores_by_extra_sections_sort_order_then_id(session) -> None:
    required = Section(code="DRILLING", name="Drill", type="production", is_active=True)
    extra = Section(code="PRESSING", name="Press", type="production", is_active=True)
...
async def test_select_route_returns_no_candidate_without_fallback(session) -> None:
    required = Section(code="DRILLING", name="Drill", type="production", is_active=True)
    other = Section(code="PRESSING", name="Press", type="production", is_active=True)
...
async def test_select_route_uses_global_and_profile_rules(session) -> None:
    wh = Section(code="RAW_STOCK", name="WH", type="raw_stock", is_active=True)
    drill = Section(code="DRILLING", name="Drill", type="production", is_active=True)
    press = Section(code="PRESSING", name="Press", type="production", is_active=True)
