...
async def test_prepare_creates_cross_ghp_transfer(client, session) -> None:
    """Если предыдущий шаг завершён, prepare на 2-м шаге cross-GHP
    должен создать Transfer (post_factum)."""
    user = await _make_user(session)
    fx = await _make_two_ghp_fixture(session, sku="PRP", qty=Decimal("3"))
    await _release_via_take_to_work(client, fx["position"].id)
    tasks = (await session.execute(select(WorkTask).order_by(WorkTask.id.asc()))).scalars().all()
    task1, task2 = tasks[0], tasks[1]

    # Завершаем 1-й шаг, чтобы на 2-м появился перемещаемый остаток.
    await _complete_first_task(client, task1.id, Decimal("3"), idempotency_key="k-prp-c")

    # После complete авто-перемещение уже создалось (Task 2). Чистим,
    # чтобы проверить именно эффект prepare.
    # NB: movements.transfer_id FK → transfers.id, поэтому сначала
    # удаляем связанные Movement-записи, иначе commit упадёт.
    from app.models.movement import Movement
    from app.models.transfer import Transfer as T
    trs = (await session.execute(select(T))).scalars().all()
    for t in trs:
        linked_movs = (await session.execute(
            select(Movement).where(Movement.transfer_id == t.id)
        )).scalars().all()
        for m in linked_movs:
            await session.delete(m)
        await session.delete(t)
    await session.commit()

    # Теперь подготовим 2-й шаг (на практике он уже существует, но
    # симулируем сценарий "если бы prepare был отдельной операцией").
    from app.transfers.services import auto_create_transfer_for_prepare
    result = await auto_create_transfer_for_prepare(
        session, to_task=task2, actor_id=user.id, idempotency_key="k-prp",
    )
    assert result is not None
    assert result.get("idempotent_replay") in (None, False)
