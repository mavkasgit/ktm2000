...
async def auto_create_transfer_for_prepare(
    db: AsyncSession,
    *,
    to_task: WorkTask,
    actor_id: int,
    ...
    """Create an automatic cross-GHP Transfer to feed a freshly prepared task.

    Called by ``prepare_section_task`` after a new WorkTask is created.
    No-op when:
    * this is the first route step (no previous section);
    * the previous section is in the same GHP;
    * the previous WorkTask has not yet completed
      (``transfer_send`` would fail on ``quantity > transferable``).

    Uses ``post_factum=True`` because the physical handover between
    sections has already happened on the shopfloor (otherwise the
    operator could not have started the previous step).
    """
    from app.models.internal_plan import SectionPlanLine
    from app.services.shopfloor.common import sections_share_spg
    ...
    if to_line is None or to_line.sequence <= 1:
        return None  # первый шаг — перемещать нечего
    ...
    if prev_task is None:
        return None  # предыдущий шаг ещё не завершён — перемещать пока нечего

    available_qty = prev_task.cached_completed_quantity
    if available_qty <= 0:
        return None

    key = f"{idempotency_key}:auto-transfer-prepare" if idempotency_key else None
    return await transfer_send(
        db,
        ...
        actor_id=actor_id,
        comment="Авто-перемещение при подготовке задания",
        idempotency_key=key,
        post_factum=True,
    )
