...
def _build_display_canon(
    ...
    return DisplayCanon(
        ...
        labels=LabelsCanon(
            ...
            quality_state_labels=dict(QUALITY_STATE_LABELS),
            stock_reason_labels=dict(STOCK_REASON_LABELS),
            section_type_labels=dict(SECTION_TYPE_LABELS),
            rule_phase_labels=dict(RULE_PHASE_LABELS),
            rule_source_labels=dict(RULE_SOURCE_LABELS),
            rule_operator_labels=dict(RULE_OPERATOR_LABELS),
            operation_code_labels=dict(OPERATION_CODE_LABELS),
            color_name_labels=dict(COLOR_NAME_LABELS),
            backup_storage_labels=dict(BACKUP_STORAGE_LABELS),
            backup_stage_labels=dict(BACKUP_STAGE_LABELS),
            backup_type_labels=dict(BACKUP_TYPE_LABELS),
            product_type_labels=dict(PRODUCT_TYPE_LABELS),
            print_column_labels=dict(PRINT_COLUMN_LABELS),
            grouping_criterion_labels=dict(GROUPING_CRITERION_LABELS),
            filter_short_labels=dict(FILTER_SHORT_LABELS),
        ),
