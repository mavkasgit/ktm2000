...
# Все склады-хранилища (raw/wip/finished/scrap) + терминал (#136): участки,
# где материал «лежит», а не обрабатывается. Это же множество проверяют
# db-триггеры (transport-операция на секции, транзитный этап маршрута).
...
# остатков — аналог Customer-локации Odoo / движения 601 в SAP.
TERMINAL_TYPES = frozenset({SECTION_TYPE_TERMINAL})

# Типы с оперативными остатками: склады-хранилища без терминала. Терминал
# (#136) вне остатков — баланс там не материализуется
# (StockProjectionManager.refresh_balance), поэтому в отчётах остатков
# участвует только это множество.
OPERATIONAL_STOCK_TYPES = STORAGE_TYPES - TERMINAL_TYPES
...
def is_storage_section(section: Section | None) -> bool:
    ...
    оборачиваемых остатков (см. ``OPERATIONAL_STOCK_TYPES``).
    """
    if section is None:
        return False
