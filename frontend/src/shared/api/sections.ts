...
export type SectionType = "production" | "raw_stock" | "wip_stock" | "finished_stock" | "scrap" | "terminal";
...
export async function fetchAllSections(params: SectionListParams = {}) {
  ...
  while (offset < total) {
    const response = await listSectionsPaginated({ limit: pageSize, offset, ...params });
