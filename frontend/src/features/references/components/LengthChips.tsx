...
export function LengthChips({ row, byLength }: { row: HangerCalcRow; byLength: Map<string, HangerCalcResult> | undefined }) {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {row.lengths.map((len) => {
        ...
        const record = (row.product.lengths ?? []).find((length) => length.length_mm === len);
        const rawLength = record ? effectiveRawLength(record) : len;
