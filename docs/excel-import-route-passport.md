...
| `ANOD` (Анодирование) | **Всегда** | Нет | **Расчёт подвесов:** `кол-во на подвес` из справочника. На подвес — только **одинаковый артикул + одинаковый цвет**. Если количество не кратно ёмкости → **округление вверх** до полного подвеса, плановое количество пересчитывается = `подвесы × ёмкость`. Пересчитанное значение записывается в строку плана с пометкой |
...
2. Выбрать: активный маршрут, количество.
