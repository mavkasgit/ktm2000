...
- Планы с `length_model_version = 1` запрещены для изменения размеров, запуска
  и операционной работы. Для продолжения переимпортировать исходный Excel новым
  планом; вручную менять версию или SQL-подменять геометрию запрещено.
- Полное удаление доступно только администратору и применимо к конкретному
  плану, не ко всей системе. Перед удалением preview показывает задействованные
  позиции, доменные строки и точные компенсации остатков. Затем операции
  откатываются mirror-проводками, задания/передачи/дефекты отменяются, а план
  скрывается из рабочих экранов. Исходный ledger, журнал действий и audit
  сохраняются; физического удаления plan/ledger-графа нет.
