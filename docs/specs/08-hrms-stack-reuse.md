...
Разобрать проект `C:\Users\user\VibeCoding\hrms` как успешный внутренний референс и решить, что стоит перенести в Factoryflow для единообразия систем и уменьшения лишней работы.

## Краткий вывод

`hrms` уже близок к стеку, который мы планировали для Factoryflow:
...
Рекомендация: Factoryflow лучше строить как технологически родственную систему, но с более строгой доменной архитектурой для производственного учета: ledger movements, release batches, immutable released positions и корректировки.
...
Для Factoryflow стоит принять такую же структуру.
...
Для Factoryflow стоит повторить этот подход.

Нужно изменить только имена:

- `hrms-postgres` -> `factoryflow-postgres`;
- `hrms_dev` -> `factoryflow_dev`;
- `hrms_test` -> `factoryflow_test`;
- `hrms_prod` -> `factoryflow_prod`;
...
Для Factoryflow стоит взять это как базу.
...
Для Factoryflow нужны аналогичные настройки:
...
Что нужно сделать строже в Factoryflow:
...
Для Factoryflow стоит использовать тот же набор.
...
Для Factoryflow важно заранее стандартизировать query keys:
...
Для Factoryflow стоит повторить это с первого milestone.
...
Для Factoryflow это особенно полезно из-за требования локальной системы.
...
Factoryflow сразу нуждается в ролях:
...
Решение для Factoryflow MVP: использовать `bigint generated always as identity primary key`.

Причины:

- Factoryflow локальный;
- один сервер;
...
Для Factoryflow этого недостаточно: производственный учет требует структурированный аудит в БД.
...
Для Factoryflow доменная логика сложнее, поэтому лучше:
...
## Что зафиксировано в Factoryflow спеках
...
1. Factoryflow использует `bigint identity` PK, а не UUID.
...
7. Бизнес-аудит идет через доменные таблицы Factoryflow; file-based audit HRMS не переносится как источник истины.
...
## Предлагаемая корректировка Factoryflow стека
