...
## Поправка (2026-09-24, ADR-0027): чтение служебной строки ≠ session lookup

Пункт 2 говорил, что break glass «bypasses users table». После ADR-0027 это
верно только про идентичность: строка `users` для break-glass по-прежнему
не обязана существовать, сессия не создаётся и не проверяется. Дополнительно
гейт резолвит **служебного пользователя** (`username = 'system'`) ради
валидного `id` для FK-колонок авторства — это чтение справочной строки, а не
session lookup, и инвариант «`sid` — только session semantics» не затронут.
